...
// object of the original type `T`, a subclass of `Serializable`.
//
...
//
// Returns an error if the `T` type from which `serialized` was produced does
// not have a corresponding `SerDes` registered or the `SerDes` returns an
// error.
template <typename T>
absl::StatusOr<std::unique_ptr<T>> Deserialize(
    const Serialized& serialized, std::unique_ptr<DeserializeOptions> options) {
  if (options != nullptr &&
      !llvm::isa<typename T::DeserializeOptions>(options.get())) {
    return absl::InvalidArgumentError("Unexpected DeserializeOptions type");
  }
  TF_ASSIGN_OR_RETURN(auto result, serdes_internal::DeserializeUnchecked(
                                       serialized, std::move(options)));
  if (!llvm::isa<T>(result.get())) {
