...
bool HandleConvolution(
  ...
  if (canonicalized_instrs.contains(conv)) {
    std::vector<int64_t> inverse_output_map =
        InversePermutation(canonicalized_instrs[conv]);
    VLOG(4) << "inverse_output_map = "
            << absl::StrJoin(inverse_output_map, ",");
  ...
  if (canonicalized_instrs.contains(conv->mutable_operand(0))) {
    std::vector<int64_t> inverse_lhs_operand_map =
        InversePermutation(canonicalized_instrs[conv->mutable_operand(0)]);
  ...
  if (canonicalized_instrs.contains(conv->mutable_operand(1))) {
    std::vector<int64_t> inverse_rhs_operand_map =
        InversePermutation(canonicalized_instrs[conv->mutable_operand(1)]);
...
bool LayoutCanonicalizer::CanonicalizeInstructionLayout(HloInstruction* instr,
  ...
  VLOG(3) << "(START) - CanonicalizeInstructionLayout: " << instr->name();

  // Check if we have previously canonicalized the instruction.
  if (canonicalized_instrs_.contains(instr)) {
    VLOG(3) << "alread canonicalized";
    return true;
  }
  VLOG(3) << "\tbefore: " << instr->ToString();

  // Make a copy of the old layout. We use the pre-canonicalization
  // major-to-minor ordering to handle each instruction.
  std::vector<int64_t> instr_old_layout;
  absl::c_copy(instr->shape().layout().minor_to_major(),
               std::back_inserter(instr_old_layout));
  ...
  // If neither output nor operands are changed, the instruction will not be
  // changed and there is no point in calling handlers.
  if (!changed_operands && !changed_output) {
    return false;
  }
  ...
  switch (instr->opcode()) {
    ...
    case HloOpcode::kConvolution:
      changed_instr =
          HandleConvolution(instr, canonicalized_instrs_, is_entry_root);
      break;
  ...
  // Store the major-to-minor ordering pre-canonicalization in the map that
  // indicate this instruction has already been canonicalized.
  std::vector<int64_t> major_to_minor;
  absl::c_reverse_copy(instr_old_layout, std::back_inserter(major_to_minor));
  canonicalized_instrs_.insert({instr, major_to_minor});

  VLOG(3) << "(END) - CanonicalizeInstructionLayout: " << instr->name();
  VLOG(3) << "\tafter: " << instr->ToString();
