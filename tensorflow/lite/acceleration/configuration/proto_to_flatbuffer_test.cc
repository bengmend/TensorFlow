...
TEST(ConversionTest, MtkNeuronSettings) {
  ...
  const uint32_t kOptimizationHint = 1;
  ...
  mtk_neuron_settings->set_optimization_hint(kOptimizationHint);
  mtk_neuron_settings->set_operation_check_mode(kOperationCheckMode);
  mtk_neuron_settings->set_allow_fp16(kAllowFp16);
  mtk_neuron_settings->set_use_ahwb(kUseAhwb);
  mtk_neuron_settings->set_use_cacheable_buffer(kUseCacheableBuffer);
  mtk_neuron_settings->set_compile_options(kCompileOptions);
  mtk_neuron_settings->set_accelerator_name(kAcceleratorName);
  ...
  EXPECT_EQ(output_mtk_neuron_settings->optimization_hint(), kOptimizationHint);
  EXPECT_EQ(output_mtk_neuron_settings->operation_check_mode(),
            MtkNeuronSettings_::OperationCheckMode_PER_NODE_OPERATION_CHECK);
  EXPECT_EQ(output_mtk_neuron_settings->allow_fp16(), kAllowFp16);
  EXPECT_EQ(output_mtk_neuron_settings->use_ahwb(), kUseAhwb);
  EXPECT_EQ(output_mtk_neuron_settings->use_cacheable_buffer(),
            kUseCacheableBuffer);
  EXPECT_EQ(output_mtk_neuron_settings->compile_options()->str(),
            kCompileOptions);
  EXPECT_EQ(output_mtk_neuron_settings->accelerator_name()->str(),
            kAcceleratorName);
